...
{
    OFAPPLOG->begin("SoundInput::setup(deviceId="+ofToString(deviceId)+",nChannels="+ofToString(nChannels)+")");
	m_useRawVol = false;
    m_smoothedVol = 0.0f;
    m_scaledVol = 0.0f;
    m_volHistoryNb = 400;
...
/*        ofLog() << "audioIn, bufferSize=" << ofToString(bufferSize) << ", nChannels=" <<ofToString(nChannels);
        string s = "";
		for (int i=0;i<bufferSize;i++){
            s+=" "+ofToString( input[i] );
        }
        ofLog() << s;
*/
...
void SoundInput::processAudio(float * input, int bufferSize, int nChannels)
{
 ...
 //       ofLog() << ofToString( m_smoothedVol );
...
//        ofLog() << ofToString( m_smoothedVol ) << " / ";
	...
	// Pitch
//    OFAPPLOG->println("m_pitch.audioIn("+ofToString(bufferSize)+","+ofToString(nChannels)+")");
